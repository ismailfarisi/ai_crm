...
const FIRST_NAMES = ['Ada', 'Grace', 'Linus', 'Rin', 'Kofi', 'Mira', 'Sana', 'Theo', 'Jun', 'Noor'];
const LAST_NAMES = ['Okafor', 'Lindqvist', 'Haddad', 'Tanaka', 'Mensah', 'Rossi', 'Novak', 'Ferreira'];
const COMPANIES = ['Cobalt Labs', 'Brightline', 'Terrafirm', 'Nimbus Health', 'Orchard Retail', 'Vantage Freight'];
const TITLES = ['Head of Ops', 'CTO', 'Procurement Lead', 'Founder', 'VP Sales', 'Finance Director'];
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    if (await orgRepo.exists({ where: { name: DEMO_ORG } })) {
      logger.warn(`"${DEMO_ORG}" already exists — nothing to seed. Run \`pnpm db:reset\` first.`);
      return;
    }
    ...
    const ownerLevel = 0;
    const teammates: { email: string; first: string; last: string; role: string }[] = [
      { email: 'admin@northwind.test', first: 'Grace', last: 'Bello', role: SYSTEM_ROLES.ADMIN },
      { email: 'manager@northwind.test', first: 'Ravi', last: 'Kapoor', role: SYSTEM_ROLES.MANAGER },
      { email: 'rep@northwind.test', first: 'Lena', last: 'Fischer', role: SYSTEM_ROLES.MEMBER },
      { email: 'viewer@northwind.test', first: 'Tom', last: 'Adeyemi', role: SYSTEM_ROLES.VIEWER },
    ];

    const created: Record<string, string> = {};
    for (const mate of teammates) {
      const role = await rbac.findRoleBySlug(organizationId, mate.role);
      const user = await users.inviteMember(organizationId, ownerLevel, {
        email: mate.email,
    ...
    const team = await teams.createTeam(organizationId, { id: ownerId, level: 0, permissions: [], isOwner: true }, {
      name: 'Enterprise Sales',
      leadId: created[SYSTEM_ROLES.MANAGER],
    });
    await users.assignTeam(organizationId, ownerId, 0, created[SYSTEM_ROLES.MEMBER], team.id);
    await users.assignTeam(organizationId, ownerId, 0, created[SYSTEM_ROLES.VIEWER], team.id);
    ...
    const owners = [ownerId, created[SYSTEM_ROLES.MANAGER], created[SYSTEM_ROLES.MEMBER]];
    ...
    for (let i = 0; i < 48; i += 1) {
      ...
      await contacts.create(actor, {
        ...
        status: CONTACT_STATUSES[i % CONTACT_STATUSES.length] as ContactStatus,
        source: CONTACT_SOURCES[i % CONTACT_SOURCES.length] as ContactSource,
        notes: i % 4 === 0 ? 'Warm intro from the Q3 conference. Follow up next week.' : '',
    ...
    logger.log('  manager@northwind.test  — leads "Enterprise Sales", sees its contacts');
