...
export class MetaWhatsAppDriver implements ChannelDriver {
  async testConnection(credentials: Record<string, any>): Promise<{ success: boolean; message: string }> {
    try {
      if (!credentials?.phoneNumberId || !credentials?.accessToken) {
        return { success: false, message: 'Missing phoneNumberId or accessToken in credentials' };
      }
      const res = await fetch(`https://graph.facebook.com/v19.0/${credentials.phoneNumberId}`, {
        headers: { Authorization: `Bearer ${credentials.accessToken}` },
      });
      const data = await res.json();
      if (data.id) {
        return { success: true, message: `Connected Meta WhatsApp Phone ID ${data.display_phone_number || data.id}` };
      }
      return { success: false, message: data.error?.message || 'Meta WhatsApp auth failed' };
    } catch (err: any) {
      return { success: false, message: err.message || 'Failed to connect to Meta WhatsApp' };
    }
  }
  ...
  async sendMessage(credentials: Record<string, any>, payload: OutboundPayload) {
    ...
    const res = await fetch(`https://graph.facebook.com/v19.0/${credentials.phoneNumberId}/messages`, {
      method: 'POST',
      headers: {
    ...
    const data = await res.json();
    if (data.error) throw new Error(data.error.message || 'Failed to send WhatsApp message');
