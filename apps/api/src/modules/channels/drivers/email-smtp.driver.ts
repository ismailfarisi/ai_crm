...
export class EmailSmtpDriver implements ChannelDriver {
    ...
    return nodemailer.createTransport({
      ...
      secure:
        credentials.secure !== undefined
          ...
          : port === 465,
      auth:
        credentials.user || credentials.auth?.user
          ? {
  ...
  async parseWebhookPayload(
    ...
    const messageBody =
      body?.text || body?.['stripped-text'] || body?.body || body?.html || '';
    return {
      senderIdentifier: String(sender),
      body: String(messageBody),
      externalMessageId:
        body?.['message-id'] || body?.messageId || body?.id
          ? String(body['message-id'] || body.messageId || body.id)
          : undefined,
      rawPayload: body,
    };
  }
